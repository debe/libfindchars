...
public final class MethodInliner {
    ...
    public void inlineMethod(CodeBuilder cb, MethodModel method, int slotBase, int depth, int maxDepth) {
        ...
        storeParamsFromStack(cb, desc, slotBase);
        ...
        emitElements(cb, codeAttr, labelMap, slotBase, nextSlotBase, depth, maxDepth,
    ...
    private static void storeParamsFromStack(CodeBuilder cb, MethodTypeDesc desc, int slotBase) {
        for (int p = desc.parameterCount() - 1; p >= 0; p--) {
            cb.storeLocal(TypeKind.from(desc.parameterType(p)), slotBase + paramSlot(desc, p));
        }
    }
    ...
    private void emitElements(CodeBuilder cb, CodeModel codeAttr, Map<Label, Label> labelMap,
            int slotBase, int nextSlotBase, int depth, int maxDepth,
            ...
            switch (elem) {
                ...
                case InvokeInstruction ii -> {
                    if (ii.opcode() == Opcode.INVOKESTATIC
                            && ii.owner().asSymbol().equals(targetOwner) && depth < maxDepth) {
                        var callee = allMethods.get(ii.name().stringValue() + ii.type().stringValue());
                        if (callee != null) inlineMethod(cb, callee, nextSlotBase, depth + 1, maxDepth);
                        else cb.with(ii);
                    } else cb.with(ii);
                }
