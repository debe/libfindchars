...
public class LiteralCompiler implements AutoCloseable{
	...
	public List<FindMask> solve(LiteralGroup... literalGroup) throws InterruptedException, SolverException {
		...
		for (int i = 0; i < literalGroup.length; i++) {
			var mask = solve1(literalGroup[i], usedLiterals);
	    ...
	    try (ProverEnvironment prover = context.newProverEnvironment(ProverOptions.GENERATE_MODELS)) {
	        ...
	        if (!isUnsat) {
	          ...
	          Map<String,Byte> literalMap = literalVectors.stream()
	            .collect(Collectors.toMap(BitvectorFormula::toString, v -> (byte) (model.evaluate(v).intValue())));
	          var result = new AsciiFindMask(lowNibblesFound, highNibblesFound, literalMap);
